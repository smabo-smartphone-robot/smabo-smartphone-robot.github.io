...
今回は、left_hand, right_handのみを有効化します。

![alt text](image-1.png)

## コントローラー制御

left_hand, right_handそれぞれ、motionを「manual」に設定してください。

<br>

次に、「Servo」タブで各サーボの角度を変更すると、それに合わせて左右の腕が動作します。
...
最初に、motionを「random」に設定してください（left_hand, right_hand両方）。

次に、random_groupを「hand」に設定してください。  
